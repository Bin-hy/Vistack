...
CREATE TABLE videos (
    ...
    cover_url TEXT COMMENT '视频封面url',
...
CREATE TABLE video_sources (
    ...
    object_path TEXT NOT NULL COMMENT '对象路径',
...
CREATE TABLE video_transcodes (
    ...
    manifest_url TEXT COMMENT 'MPD 文件地址',
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE video_segments (
    id BIGSERIAL PRIMARY KEY,
    transcode_id BIGINT NOT NULL REFERENCES video_transcodes(id) ON DELETE CASCADE,
    uuid UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    segment_index INT NOT NULL COMMENT '分片索引',
    duration FLOAT COMMENT '分片时长',
    segment_url TEXT NOT NULL COMMENT '分片url'
);
