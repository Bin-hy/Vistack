...
<template>
	<div class="relative flex items-center text-xs text-white">
		<button
			...
			@click="isOpen = !isOpen"
		>
			<span class="mr-1">清晰度</span>
			<span>{{ currentLabel }}</span>
		</button>
		<div
			v-if="isOpen"
			class="absolute bottom-8 right-0 z-20 w-24 rounded bg-black/90 py-1 text-xs shadow-lg"
		>
			<button
				type="button"
				class="block w-full px-3 py-1 text-left"
				:class="props.value === 'auto' ? 'text-[#00A1D6]' : 'text-white/80 hover:text-white'"
			...
			<button
				...
				type="button"
				class="block w-full px-3 py-1 text-left"
				:class="props.value === q.index ? 'text-[#00A1D6]' : 'text-white/80 hover:text-white'"
